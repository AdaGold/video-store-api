...
describe RentalsController do
  ...
  describe 'check_out' do
    it "creates a new rental due one week from today" do
      ...
      new_rental = Rental.find_by(customer_id: customer_id, video_id: video_id)
  ...
  describe 'check_in' do
    it "adjusts video available inventory and customer checked in count" do
      existing_rental = rentals(:valid_rental)
      customer = existing_rental.customer
      customer_id = customer.id
      customer_original_count = customer.videos_checked_out_count

      video = existing_rental.video
      video_id = video.id
      video_original_available_inventory = video.available_inventory
      rental_data = {
        customer_id: customer_id,
      ...
      expect(body.keys).must_equal ["id", "customer_id", "video_id", "videos_checked_out_count", "available_inventory"]

      rental = Rental.find_by(customer_id: customer_id, video_id: video_id)
      customer.reload
      expect(customer.videos_checked_out_count).must_equal (customer_original_count - 1)
      video.reload
      expect(video.available_inventory).must_equal (video_original_available_inventory + 1)
